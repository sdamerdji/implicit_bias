...
def stage_i(frame, i):
    ...
    def new_center(new):
        center['text'] = new
    #ttk.Label(frame, text="", wraplength = 500).grid(column=1,row=2) # Add x if they mess up
...
def clear(container):
    for widgets in container.winfo_children():
        widgets.destroy()
    #map(Widget.destroy, container.grid_slaves())
...
try:
    ...
    with open(FILENAME, 'w') as myfile:
        myfile.write('time,gender,factor,1_avg,2_avg,3_avg,4_avg,5_avg')
